...
class ExampleRegistry
{
    ...
    private function define(): array
    {
        return [
            ...
            new Example(
                id: 'relation-aggregates',
                category: 'Relationships',
                title: 'Aggregates over a plural relation',
                description: 'How many orders, how much they came to, whether there are any — as columns, sorts and filters.',
                table: DynamicTables\RelationAggregatesTable::class,
                notes: [
                    'The spelling is Eloquent’s own — orders_count, orders_sum_total, orders_max_placed_at, orders_exists — because these compile to withCount(), withAggregate() and withExists(). The name you write is the name the query answers with.',
                    'One correlated subselect each: no join, no GROUP BY on the outer query, and no row multiplied by its relations.',
                    'A real column always wins. A model with a denormalised orders_count counter cache keeps reading that column — it is there, it is indexed, and reading it is free.',
                    'They sort, and they filter: the filter builder offers _count and _exists for every plural relation, and a declared aggregate column is filterable like any other.',
                    'sum, avg and count treat "no related rows" as zero, so "spend under 100" includes a customer who has never ordered. min and max over no rows are null.',
                    'Search is the one thing they cannot do: it compiles to a WHERE on a column, and a subquery alias cannot be named in a WHERE clause.',
                ],
                keywords: ['count', 'sum', 'avg', 'exists', 'hasmany', 'withcount', 'aggregate', 'subquery'],
            ),
            new Example(
            ...
            new Example(
                ...
                notes: [
                    ...
                    '"Shipped before it was placed" compares two columns rather than a column and a value: {"field": "placed_at"} in place of the value. This is how the data-quality questions get asked — rows that contradict themselves — and it compiles to whereColumn, so it costs nothing extra.',
                    'Both sides have to be real columns on the table’s own row and of the same type family. A relation path, an aggregate or "date before name" is refused and dropped with a warning, like any other invalid condition.',
                ],
                keywords: ['preset', 'default view', 'code', 'column comparison', 'wherecolumn'],
            ),
            ...
            new Example(
                id: 'group-summaries',
                category: 'Columns',
                title: 'Grouping, with subtotals',
                description: 'Group by a column and every heading carries its own totals.',
                table: DynamicTables\GroupSummariesTable::class,
                notes: [
                    'Open the header menu on any column and choose Group by. The heading rows gain the subtotals for their own group, and the row under the table keeps the total for the whole filtered result.',
                    'The number on a heading is the whole group’s, not the visible slice’s: a group cut in half by a page break still reports what the group holds, because that is what the heading claims to describe.',
                    'One extra aggregate query, bounded to the groups on the page — grouping a large table by a high-cardinality column does not aggregate every group in the result to label the handful you can see.',
                    'Grouping is expressed as a leading sort, so the database does the work and the browser only notices where the value changes. Nothing is loaded into PHP to be grouped.',
                    'Grouping by a computed or relationship column gets no subtotals: there is no single column to GROUP BY without a join.',
                ],
                keywords: ['group', 'subtotal', 'summary', 'total', 'aggregate', 'heading'],
            ),
            new Example(
                id: 'computed-columns',
                category: 'Columns',
            ...
            new Example(
                ...
                notes: [
                    ...
                    'Two people editing one row no longer overwrite each other. Each row carries a version derived from updated_at, and an edit written against a version that has moved on is refused — "someone else changed this row" — with the row handed back as it now stands, so you can see what you are deciding between.',
                    'Nothing to switch on, and no column to add: a model without timestamps has no version and edits exactly as it always did.',
                ],
                keywords: ['edit', 'inline', 'cell', 'save', 'concurrency', 'conflict'],
            ),
            new Example(
                id: 'validation',
                category: 'Editing',
            ...
            new Example(
                ...
                notes: [
                    ...
                    '**Preview** answers the question you actually have in front of a mapping screen — what is this about to do to my data? It is not a simulation: it is the real import inside a transaction that is rolled back, so the same validation, casting, lookups and constraints produce the same numbers, and the rejected rows come back with their report before anything is written.',
                    'A preview never queues, whatever the size — answering "what will this do?" with "ask again later" is not an answer. The upload survives it, so Start import is still the next thing to press.',
                    'What a preview cannot undo is what leaves the database: a model observer that queues a job or sends a mail still did so.',
                    'The mapping is remembered for the next upload, keyed by heading name rather than position — a file that has gained a column keeps every decision already made about the others.',
                ],
                keywords: ['import', 'upload', 'mapping', 'csv', 'excel', 'template', 'preview', 'dry run'],
            ),
            ...
            new Example(
                ...
                description: 'rowUrl() makes the first cell a real link, and the rest of the row follows it.',
                table: DynamicTables\RowLinksTable::class,
                notes: [
                    'The first cell is a genuine anchor: focusable, announced as a link, middle-clickable, and "copy link address" works. A click handler gives you none of that, which is why it is only the convenience on top.',
                    '$rowClick chooses single, double or none. The default is "double" on a table with inline editing — double-click already opens the editor there — and "single" everywhere else.',
                    'A click never navigates from an editable cell, from anything already interactive, or when it ended a text selection.',
                    'Ctrl, ⌘ or Shift opens a tab, the way a link would.',
                ],
                keywords: ['link', 'click', 'navigate', 'row url', 'drilldown'],
            ),
            new Example(
                id: 'pinned-rows',
                category: 'UI',
                title: 'Pinned rows',
                description: 'Star a row and it stays at the top — for you, and for nobody else.',
                table: DynamicTables\PinnedRowsTable::class,
                notes: [
                    'Pin something on page 4 and it is at the top of page 1, exactly once. It is one ORDER BY CASE rather than a union or a second query, so there is still one result, one count and one page.',
                    'Pinned rows sort above everything, the group heading included.',
                    'The list lives in the session, like remembered state and for the same reason: a pin is a working note, not a document. Saved views are the durable, shareable version of the same idea.',
                    'Up to 50, oldest dropped first — an unbounded list in the session would be an unbounded IN clause in the query.',
                ],
                keywords: ['pin', 'star', 'bookmark', 'favourite', 'top'],
            ),
            new Example(
                id: 'row-reorder',
                category: 'UI',
                title: 'Reordering rows',
                description: 'Drag a row by its grip; the position column is written on drop.',
                table: DynamicTables\ReorderTable::class,
                notes: [
                    'The grips are only there while the table is sorted by the position column. Sort by Name from the header menu and they disappear — under any other sort, dropping a row between two others describes a position the table is not showing. The server refuses such a drag as well.',
                    'A drag does not renumber anything: the positions on the page are a set of slots, and the drag permutes which row sits in which. So a drag on page 40 costs what a drag on page 1 costs, and no new value can collide with a row you cannot see.',
                    'Every record is re-fetched through the table’s own base query and checked for "update", so a drag can never move a row this table would not have shown.',
                    'A refused drag is not half-applied — the whole reorder is rejected and the table refetches, rather than leaving an order that is neither the old one nor the one you asked for.',
                ],
                extraFiles: ['database/migrations/2026_01_01_000300_add_position_to_categories.php'],
                keywords: ['drag', 'sort order', 'position', 'reorder', 'manual order'],
            ),
            new Example(
                id: 'slots',
                category: 'UI',
                title: 'Slots',
                description: 'Your own markup in the five places the template leaves for it.',
                table: DynamicTables\SlotsTable::class,
                notes: [
                    'toolbar.start, toolbar.end, above, below and empty. Each one is somewhere the package promises not to draw anything itself, so your markup and the table’s can never fight over the same box.',
                    'Values take the same three shapes as rowDetail(): a Blade view, an Htmlable, or a string — which is escaped, because a bare string is text. Look at the line under the table.',
                    'An unknown slot name throws, for the same reason an unknown feature name does: markup rendered nowhere, with nothing anywhere saying why.',
                    'The empty slot appears only when the table is genuinely empty, never when a filter matched nothing — "add the first order" answers a question the reader did not ask.',
                    'This is the supported alternative to copying table.blade.php, which works once and turns every upgrade into a merge.',
                ],
                extraFiles: [
                    'resources/views/partials/slot-toolbar.blade.php',
                    'resources/views/partials/slot-empty.blade.php',
                ],
                keywords: ['slot', 'toolbar', 'banner', 'custom markup', 'blade'],
            ),
            new Example(
                id: 'custom-theme',
                category: 'UI',
            ...
            new Example(
                ...
                notes: [
                    ...
                    'It pages by **key, not by offset**: each page asks for the rows *after* the last ones. With OFFSET, a row inserted above the window shifts everything down — you see one row twice and never see another — and page 400 costs the database 10,000 rows it throws away. A cursor has neither problem.',
                    'The cursor is opaque, never written to the URL and never saved in a view: it points into one particular result and means nothing in another.',
                    'A sort the cursor cannot express — on a relationship or a relation aggregate, both subqueries — falls back to offset paging, silently and correctly.',
                ],
                keywords: ['infinite', 'scroll', 'lazy', 'append', 'pagination', 'cursor', 'keyset'],
            ),
            new Example(
                id: 'performance',
                category: 'Performance',
